...
module.exports = class FirehoseWriter {
  ...
  */
  constructor(options = {}) {
    ...
    if (this.maxSize > AWS_MAX_RECORD_SIZE * AWS_MAX_BATCH_RECORDS) {
      throw new Error(`maxSize should be at most ${AWS_MAX_RECORD_SIZE * AWS_MAX_BATCH_RECORDS}`)
    }

    this.maxCount = options.maxCount || 500
    if (this.maxCount > AWS_MAX_BATCH_RECORDS) {
      throw new Error(`maxCount should be at most ${AWS_MAX_BATCH_RECORDS}`)
    }
    ...
    if (this.maxBatchCount > AWS_MAX_BATCH_RECORDS) {
      throw new Error(`maxBatchCount should be at most ${AWS_MAX_BATCH_RECORDS}`)
    }
    this.maxBatchSize = options.maxBatchSize || AWS_MAX_RECORD_SIZE * AWS_MAX_BATCH_RECORDS
    if (this.maxBatchSize > AWS_MAX_RECORD_SIZE * AWS_MAX_BATCH_RECORDS) {
      throw new Error(`maxBatchSize should be at most ${AWS_MAX_RECORD_SIZE * AWS_MAX_BATCH_RECORDS}`)
    }
